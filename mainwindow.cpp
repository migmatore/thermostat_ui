...
{
    ui->setupUi(this);
    if (bme280Init(0, 0x76) != 0)
    {
        qWarning("error bmp280 init"); // problem - quit
    }

    qWarning("BME280 device successfully opened.\n");
    usleep(1000000);
    timer = new QTimer();
...
void MainWindow::update() {
    ...
    qWarning("Calibrated temp. = %3.2f C, pres. = %6.2f Pa, hum. = %2.2f%%\n", (float)T/100.0, (float)P/256.0, (float)H/1024.0);
    QString test = QVariant(T/100).toString();
    ui->label->setText(QString::number((float)T/100.0, 'f', 2));
    //usleep(DELAY);
